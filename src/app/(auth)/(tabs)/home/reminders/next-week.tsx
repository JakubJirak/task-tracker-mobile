...
export default function RemindersNextWeek() {
  const { nextWeekReminders, isLoading, isError } = useReminders();
        ...
        <ScrollView className="mt-3" showsVerticalScrollIndicator={false}>
          <View className="gap-2 pb-24">
            <Text className="text-text">
              Příští týden ({nextWeekReminders.length})
            </Text>
            {nextWeekReminders.map((event) => (
